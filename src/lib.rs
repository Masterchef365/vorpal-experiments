...
extern "C" {
    fn kernel(ptr: *mut f32, width: f32, height: f32, x: f32, y: f32, time: f32, cursor_x: f32, cursor_y: f32);
}

pub fn call_kernel(width: f32, height: f32, x: f32, y: f32, time: f32, cursor_x: f32, cursor_y: f32) -> [f32; 4] {
    let mut out_data = [0_f32; 4];

    unsafe {
        kernel(out_data.as_mut_ptr(), width, height, x, y, time, cursor_x, cursor_y);
    }
...
#[no_mangle]
pub extern "C" fn make_image(width: u32, height: u32, time: f32, cursor_x: f32, cursor_y: f32) -> *const f32 {
    thread_local! {
